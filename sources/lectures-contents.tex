...
\begin{exercise}
Let $X \sim \text{Pois}(\lambda)$ and $Y \sim \text{Pois}(\mu)$ be independent. What is the distribution of $Z = X + Y$?\\
\begin{hint}
...
\end{align}
We recognize this as the PMF of a Poisson distribution with parameter $\lambda+\mu$. Hence, $Z \sim \text{Pois}(\lambda + \mu)$.
\end{solution}
\end{exercise}

\begin{exercise}
(Theorem 8.4.3 in the book.) Let $X_1, X_2, \ldots$ be i.i.d. $\text{exp}(\lambda)$ distributed. Let $T_n = \sum_{k=1}^n X_k$. Show that $Y_n$ has the following pdf:
\begin{align}
    f_{T_n}(t) = \frac{\lambda^n}{(n-1)!} t^{n-1} e^{-\lambda t}, \quad t > 0.
\end{align}
That is, show that $T_n$ follows a \textit{Gamma distribution} with parameters $n$ and $\lambda$. (We will learn about the Gamma distribution in chapter 8.4 in the book.)\\
...
\begin{solution}
Recall the pdf of a $\mathcal{N}(\mu,\sigma)$ random variable $T$:
\begin{align}
    f_T(t) = \frac{1}{\sqrt{2\pi}\sigma} \exp\left\{-\frac{(t-\mu)^2}{2 \sigma}\right\}, \quad t \in \R.
\end{align}
...
\begin{align}
    ...
    &= \frac{1}{\sqrt{2\pi} \cdot 2} \exp\left\{-\frac{z^2}{2 \cdot 2}\right\} \cdot \int_{-\infty}^\infty  \frac{1}{\sqrt{2\pi} \cdot (1/2)} \exp\left\{-\frac{\left(x - \tfrac{1}{2}z\right)^2}{2 \cdot (1/2)}\right\} dx \\
