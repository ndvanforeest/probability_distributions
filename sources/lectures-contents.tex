...
Compute $\E{X \given X\leq \tau}$ where $\tau$ is some positive constant. Then use  Adam's law and LOTP to show that $\E{X} = \E{\E{X\given X\geq \tau}} = 1/\lambda$. (Observe that this is just a check!)
\begin{solution}
Since $X\sim \Exp{\lambda}$,
\begin{equation*}
\P{X\leq t | X\leq \tau} = \frac{\P{X\leq \min\{t, \tau\}}}{\P{X\leq \tau}} = \frac{1-e^{-\lambda\min\{t, \tau\}}}{1-e^{-\lambda \tau}}.
\end{equation*}
Take $t\leq \tau$ for ease, then define
\begin{equation*}
  f(t) = \partial_{t} \P{X\leq t | X\leq \tau} = \lambda \frac{e^{-\lambda t}}{1-e^{-\lambda \tau}}.
\end{equation*}
...
\begin{align}
  \label{eq:25}
\E{X|X\leq \tau}
&= \frac{1}{1-e^{-\lambda t}}\int_{0}^{\tau} \lambda t e^{-\lambda t} \d t\\
%&= \frac{1}{1-e^{-\lambda t}}\int_{0}^{\tau} \lambda t e^{-\lambda t} \d t\\
\end{align}
Since
\begin{align}
...
\end{align}
By memorylessness, $\E{X\given X>\tau} = \tau + 1/\lambda$. Hence, taking $Y=\1{X\leq \tau}$
\begin{align}
  \label{eq:4}
\E X
&= \E{\E{X\given Y}}, \quad \text{Adam} \\
...
Here is a final point for you to think hard about: ${X\leq \tau}$ is an event, $Y=\1{X \leq \tau}$ is a r.v. Therefore we \emph{should not} write $\E{\E{X\given X\leq \tau}}$.
...
\begin{exercise}
We have two machines, one is working, the other is off.
If the first fails, the other machine takes over.
The repair time of the first machine is a constant $\tau$.
If the second machine fails before the first is repaired, the station (containing the two machines) is `down'.
...
\begin{equation}
  \label{eq:27}
\E{T| X_{2}\leq \tau} = \E{X_{1} + X_{2}  |X_2\leq \tau} = 1/\lambda + \frac{ 1- e^{-\lambda \tau}- \tau \lambda  e^{-\lambda \tau}}{\lambda(1-e^{-\lambda \tau})}.
\end{equation}
...
Find a recursion to compute the expected number $T_{0}$ of draws necessary to see all balls.
\begin{solution}
Write $T_{n}$ for expected time to finish given that we have seen $n$ different balls. Then,
\begin{align}
\label{eq:28}
T_n
&= 1
...
That's obvious: $T_{N} = 0$. Hence, $T_{N-1} = N$, and so on.

If you like (but just skip it, if you already happy at this point) you can push this a bit further: approximate $T_{0}$ for $N\gg 0$.
\end{solution}
\end{exercise}
...
\begin{solution}
  ...
  Take 6 balls.
  If we would now the number $k$ of new balls drawn, then $T_{n} = 1 + T_{n+k}$.
  What is the probability to draw $k$ new balls (assuming for ease that $k<N -n$), out of the 6 we pick?
  This must be
\begin{equation}
\label{eq:30}
{n \choose 6-k}{N-n \choose k}\big/{N \choose 6}.
\end{equation}
And therefore, by LOTE,
\begin{align}
T_{n}
&=
...
\begin{exercise}
For the previous two exercises, compute $T_{0}$ for $N=45$.
\begin{solution}
Here is the python code. First for the case in which we draw one ball at a time.
\begin{pyblock}
N = 45
...
print(T(0))
\end{pyblock}
Here are the results $T(44) = \py{T(44)}$ (This is a simple check) and $T{0} = \py{T(0)}$.


For the other exercise, in which we draw $m=6$ balls at a time, the recursion is considerably harder, unless we use a concept called \emph{memoization}.
In short, this means that we check if we computed the value of a function earlier.
If so, get the value, otherwise, do the computation, and store it for later purposes.
Without memoization, the computation below takes a (very) long time, but with memoization it's a matter of milliseconds. You should memorize this concept of memoization.
\begin{pyblock}
...
(Note that I implemented the code for a general $m$ so that I could compare for $m=1$ with our earlier results.
Hopefully you learn from this that you should also test your code.)
Next,  $T(0, 6) = \py{T(0,6)}$.
This is more than $6$ as fast, i.e., $6T(0, 6) = \py{6*T(0, 6)}$. Why is that? Well, the 6 balls we pick are guaranteed to be drawn without replacement.

What should $T(N,N)$ be? (I used also this case to test my code.)

Suppose for each time you draw a set of balls, you get the value (in Euro's say) of the numbers on the balls, and then you put the balls back in the urn.
