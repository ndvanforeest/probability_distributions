...
Let $P\in \{A, B\}$ be rv that says whether selected player is the arbiter or not, and $S\in\{0,1\}$ be the score.
\begin{enumerate}
\item What is the PMF, in other words, determine $\P{P = A, S=1}$ and so on for all possibilities?
...
For our present example, let's check with for a particular outcome:
\begin{align*}
...
\begin{exercise}
Find an expression for $F_{Z}(k)$ for $Z=X+Y$.
\begin{solution}
