...
You should memorize this concept when dealing with the compuation of recursions.
...
\begin{align}
  \label{}
\V{Y\given X} &= X^{2}/12, \\
\E{\V{Y\given X}} &= \E{X^2}/12 = \frac{1}{12} \int_0^1x^2 \d x = \frac{1}{12\cdot 3}\\
\E{Y\given X} &= X/2,\\
\V{\E{Y\given X}} &= \V{X/2} = \frac{1}{4\cdot12}, \\
...
\begin{align}
  ...
  \end{cases}
\end{align}
and $0$ elsewhere. Define the r.v. $Y$ such that $Y=\1{X< 1} + 2\1{x\geq 1}$. Explain what it means that $Y=1$ or $Y=2$.
Compute $\E X$ and $\V X$. Then use conditioning on $Y$ and Adam's and Eve's laws to recompute $\E X$ and $\V X$.
\begin{solution}
When $Y=1$, then necessarily $X<1$, and similar for $Y=2$.
...
\E{X \given Y} &= \frac{1}{2} \1{Y=1} + 2 \1{Y=2} \\
\E{X} &= \E{\E{X\given Y}} = \E{\frac{1}{2} \1{Y=1} + 2 \1{Y=2}} = \frac{1}{2} \frac{1}{2} + 2 \frac{1}{2} = \frac{5}{4},\\
\V{X\given Y} &= \V{X\given Y=1}\1{Y=1} + \V{X\given Y=2}\1{Y=2} = \frac{1}{12} \1{Y=1} + \frac{4}{12} \1{Y=2}, \\
...
While the server works on the job, it gets interrupted to do other tasks $\{R_{i}\}$.
...
\begin{solution}
Use the properties of the Poisson distribution.
\begin{align}
\E{N\given T} &= \lambda T \\
\V{N\given T} &= \lambda T \\
\E N &= \E{\E{N\given T}} = \lambda \E{T}\\
\E{\V{N\given T}} &= \lambda \E{T} \\
\V{\E{N\given T}} &= \V{\lambda T} = \lambda^{2} \V T \\
\V N &= \lambda \E T + \lambda^{2} \V T.
\end{align}

The  duration $S$ of a job  is its own service time $T$ plus all interruptions, hence,
\begin{align}
  \label{}
\E S
&= \E{T + \sum_{i=1}^{N} R_{i}} \\
&= \E T + \E{\sum_{i=1}^{N} R_{i}} \\
&= \E T + \E{\E{\sum_{i=1}^{N} R_{i}\given N}} \\
&= \E{T} + \E {N \E R} \\
&= \E T + \lambda \E T \E R \\
&= (1+\lambda \E R)\E T.
\end{align}
...
\V S
&= \V{T + \sum_{i=1}^{N} R_{i}} \\
&= \V{T} + \V{\sum_{i=1}^{N} R_{i}}, \quad \textrm{use Eve's law},\\
&= \V{T} + \E{\V{\sum_{i=1}^{N} R_{i}\given N}} + \V{\E{\sum_{i=1}^N R_{i}\given N}}\\
&= \V{T} + \E{N \V R} + \V{N \E R} \\
&= \V{T} + \lambda \E T \V R + (\E R)^{2} \V N \\
&= \V{T} + \lambda \E T \V R + (\E R)^{2} (\lambda \E T + \lambda^{2}\V T)
\end{align}
