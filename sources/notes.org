...
\begin{align*}
\P{X=2} = \int \P{X=2\given \lambda} f_{\Lambda}(\lambda) \d \lambda = \int_0^{\infty} e^{-2\lambda} (2\lambda)^{2}/2! \cdots 3 e^{-3\lambda} \d \lambda,
\end{align*}
and simplify all.
...
Then, use independence to simplify $\E{e^{s T}\given N = n}$ to $\left(\E{e^{s X_{1}}}\right)^n$. Then, $f(s) = \E{e^{s X_1}} = p e^s + 1-p$. Use the tower property to get rid of the condition on $N$, and use the geometric series to simplify. Finally, writing $M(s) = \E{e^{s t}}$, realize that $\E T = M'(0)$, and $\E{T^2} = M''(s)$. With this, check the results of a.
