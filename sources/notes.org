...
Here is the answer.
\begin{align*}
f_{T} = \frac 1{t^2} \int_0^{\infty} f_X(x) f_Y(t x)\d x.
\end{align*}
...
Take $V=Y-c X$. We know from Ch 8 that $V$ is normally distributed. Therefore, when $\cov{X,V} = 0$, $X$ and $V$ are independent.
